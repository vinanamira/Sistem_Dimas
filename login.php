...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
  ...
  <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
    ...
    <div id="errorBox" class="hidden mb-4 p-3 bg-red-100 text-red-700 rounded text-sm"></div>
    ...
    <form id="loginForm" method="POST" action="backend/proses/login.php">
      <div class="mb-4">
        <label for="username" class="block text-sm font-medium text-gray-700 mb-1">Username / NIS</label>
    ...
    document.getElementById('loginForm').addEventListener('submit', async function(e) {
      ...
      try {
        ...
        if (data.success && data.session_id) {
          sessionStorage.setItem('tab_session_id', data.session_id);
          window.location.href = data.redirect;
        } else {
          errorBox.textContent = data.error || 'Terjadi kesalahan saat login.';
